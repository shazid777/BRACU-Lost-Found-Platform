...
from data import add_notification, get_notifications
from data import add_rating, get_user_ratings, calculate_average_rating
from data import find_matches
...
@app.route('/rate_user/<int:rated_user_id>', methods=['GET', 'POST'])
@login_required
def rate_user(rated_user_id):
    print(f"Rendering rate_user.html for user ID {rated_user_id}")

    if request.method == 'POST':
        rating = int(request.form['rating'])
        feedback = request.form['feedback']
        add_rating(current_user.id, rated_user_id, rating, feedback)
        flash("Rating submitted successfully!", "success")
        return redirect(url_for('user_profile', user_id=rated_user_id))

    return render_template('rate_user.html', rated_user_id=rated_user_id)
...
def user_profile(user_id):
    ...
    ratings = get_user_ratings(user_id)  # List of all ratings
    average_rating = calculate_average_rating(user_id)  # Average rating value

    return render_template('user_profile.html', user=user, ratings=ratings, average_rating=average_rating)

@app.route('/users', methods=['GET'])
@login_required
def list_users():
    conn = get_db_connection()
    users = conn.execute("SELECT id, first_name, email FROM users").fetchall()
    conn.close()
    return render_template('list_users.html', users=users)

@app.route('/view_all_matches', methods=['GET'])
@login_required
def view_all_matches():
    # Call the find_matches function to compute matches
    matches = find_matches()

    # Fetch matches from the database to display
    conn = get_db_connection()
    all_matches = conn.execute("""
        SELECT 
            ...
            lost.last_seen_location AS lost_location,
            found.last_seen_location AS found_location
        FROM matches m
        JOIN posts lost ON m.lost_post_id = lost.id
        JOIN posts found ON m.found_post_id = found.id
        ORDER BY m.similarity DESC
    """).fetchall()
    conn.close()

    return render_template('view_all_matches.html', matches=all_matches)
