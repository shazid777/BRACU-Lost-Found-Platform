...
from data import insert_claim, get_claims_for_admin  # Import the new function
from data import get_lost_posts, get_found_posts
...
def home():
    if current_user.is_admin:
        return redirect(url_for('admin_dashboard'))  # Redirect to admin management page

    from data import get_sorted_posts
    sort_by = request.args.get('sort_by', 'recent')
    lost_posts = get_sorted_posts("lost", sort_by)
    found_posts = get_sorted_posts("found", sort_by)
    return render_template('home.html', lost_posts=lost_posts, found_posts=found_posts, sort_by=sort_by)
...
def claim_item_route(item_id):
    if request.method == 'POST':
        ...
        try:
            ...
            claim_id = insert_claim(item_id, user_id, question_1, question_2, question_3, '')

            if claim_id:
                print(f"Claim submitted successfully with ID: {claim_id}")  # Debugging line
...
def manage_reports():
    ...
    from data import get_all_reports, delete_post
    ...
    return render_template('admin_manage_reports.html', reports=reports)
