...
# Create notifications table tanzila
    cursor.execute("""
    ...
    """)
###matches table tanzila
    cursor.execute("""
    CREATE TABLE IF NOT EXISTS matches (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    lost_post_id TEXT NOT NULL,
    found_post_id TEXT NOT NULL,
...
def insert_found_post(title, description, last_seen_location, date, photo):
    ...
    try:
        ...
        INSERT INTO posts (id, title, description, category, last_seen_location, date, photos, post_type)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (post_id, title, description, 'Other', last_seen_location, date, photo, 'found'))  # Default category
...
def add_rating(rater_id, rated_user_id, rating, feedback):
    """Add a new rating."""
    conn = get_db_connection()
    conn.execute(
        "INSERT INTO ratings (rater_id, rated_user_id, rating, feedback) VALUES (?, ?, ?, ?)",
        (rater_id, rated_user_id, rating, feedback)
    )
    conn.commit()
    conn.close()
...
def get_user_ratings(user_id):
    ...
    ratings = conn.execute(
        "SELECT rating, feedback, created_at FROM ratings WHERE rated_user_id = ?",
        (user_id,)
    ).fetchall()
    conn.close()
    return ratings
...
def find_matches():
    """Find matches between lost and found posts and save them to the matches table."""
    conn = get_db_connection()

    # Fetch all lost and found posts
    lost_posts = conn.execute("SELECT * FROM posts WHERE post_type = 'lost'").fetchall()
    found_posts = conn.execute("SELECT * FROM posts WHERE post_type = 'found'").fetchall()

    matches = []

    for lost in lost_posts:
        for found in found_posts:
            # Compare keywords in title, description, and location
            title_similarity = SequenceMatcher(None, lost['title'], found['title']).ratio()
            description_similarity = SequenceMatcher(None, lost['description'], found['description']).ratio()
            location_similarity = SequenceMatcher(None, lost['last_seen_location'], found['last_seen_location']).ratio()

            # Set thresholds for matching
            if title_similarity > 0.4 and description_similarity > 0.4 and location_similarity > 0.4:
                similarity = (title_similarity + description_similarity + location_similarity) / 3  # Average similarity

                # Save match to the matches table
                conn.execute(
                    "INSERT INTO matches (lost_post_id, found_post_id, similarity) VALUES (?, ?, ?)",
                    (lost['id'], found['id'], similarity)
                )

                # Add match to the results
                matches.append({
                    "lost_post_id": lost['id'],
                    "found_post_id": found['id'],
                    "similarity": similarity,
                    "lost_title": lost['title'],
                    "found_title": found['title'],
                    "lost_location": lost['last_seen_location'],
                    "found_location": found['last_seen_location'],
                })
